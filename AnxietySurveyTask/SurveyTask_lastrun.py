...
def setupData(expInfo, dataDir=None):
    ...
    if dataDir is None:
        dataDir = _thisDir
    filename = u'data/%s-%s_%s_%s' % (expInfo['participant'], expInfo['session'], expName, expInfo['date'])
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/brainlab/Documents/GitHub/Attentional_Switching_EEG/AnxietySurveyTask/SurveyTask_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    form = visual.Form(win=win, name='form',
        items='/Users/brainlab/Documents/GitHub/Attentional_Switching_EEG/AnxietySurveyTask/SCAARED_form.xlsx',
        textHeight=0.03,
        font='Open Sans',
        randomize=False,
        style='dark',
    ...
    # reset done_button to account for continued clicks & clear times on/off
    done_button.reset()
    ...
    while continueRoutine:
        ...
        # if done_button is starting this frame...
        if done_button.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'done_button.started')
            # update status
            done_button.status = STARTED
            done_button.setAutoDraw(True)
        
        # if done_button is active this frame...
        if done_button.status == STARTED:
            ...
            if done_button.isClicked:
                if not done_button.wasClicked:
                    ...
                    done_button.timesOn.append(done_button.buttonClock.getTime())
                    done_button.timesOff.append(done_button.buttonClock.getTime())
                elif len(done_button.timesOff):
                    # if click is continuing from last frame, update time of clicked until
                    done_button.timesOff[-1] = done_button.buttonClock.getTime()
                if not done_button.wasClicked:
                    # run callback code when done_button is clicked
                    if form.complete:
                        ...
                        if np.array_equal(footer.color, [1,1,1]):
                            ...
                            footer.color = 'white'
                        continueRoutine = True
        # take note of whether done_button was clicked, so that next frame we know if clicks are new
        done_button.wasClicked = done_button.isClicked and done_button.status == STARTED
        
        # *footer* updates
        
        # if footer is starting this frame...
        if footer.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'footer.started')
            # update status
            footer.status = STARTED
            footer.setAutoDraw(True)
        
        # if footer is active this frame...
        if footer.status == STARTED:
            # update params
            pass
